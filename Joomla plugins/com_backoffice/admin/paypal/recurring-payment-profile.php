...
if($rp_invoice_id != '')
{
	...
	$input = $db -> escape($rp_invoice_id);
	$result_rows = $model->getRecurringPaymentProfile1($input);
...
else
{
	...
	$input = $db -> escape($recurring_payment_id);
	$result_rows = getRecurringPaymentProfile2($input);
	foreach($result_rows as $result_row)
		$profile_exists = true;
}
